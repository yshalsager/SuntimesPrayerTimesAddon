...
const val fallback_host_event_authority = "com.test.fallback.event.provider"
const val fallback_host_calc_authority = "com.test.fallback.calculator.provider"
const val offday_host_event_authority = "com.test.offday.event.provider"
const val offday_host_calc_authority = "com.test.offday.calculator.provider"
...
class FallbackOnlyHostEventProvider : ContentProvider() {
    override fun onCreate(): Boolean = true
...
class OffdaySunHostCalcProvider : ContentProvider() {
    override fun onCreate(): Boolean = true
    ...
    private fun query_config(projection: Array<String>?): Cursor {
        ...
        cols.indices.forEach { i ->
            row[i] =
                when (cols[i]) {
                    CalculatorConfigContract.column_location -> "Offday Test"
                    CalculatorConfigContract.column_latitude -> "21.4"
                    CalculatorConfigContract.column_longitude -> "39.8"
                    CalculatorConfigContract.column_timezone -> "UTC"
                    else -> null
                }
        }
    ...
    private fun query_sun(at_millis_segment: String?, projection: Array<String>?): Cursor {
        ...
        val at_millis = at_millis_segment?.toLongOrNull() ?: System.currentTimeMillis()
        val day_start = at_millis - Math.floorMod(at_millis, day_millis)
        val previous_day_start = day_start - day_millis

        val row = arrayOfNulls<Any>(cols.size)
        cols.indices.forEach { i ->
            row[i] =
                when (cols[i]) {
                    CalculatorConfigContract.column_sun_noon -> previous_day_start + 12 * 60 * 60 * 1000L
                    CalculatorConfigContract.column_sunrise -> previous_day_start + 6 * 60 * 60 * 1000L
                    CalculatorConfigContract.column_sunset -> previous_day_start + 18 * 60 * 60 * 1000L
                    else -> null
                }
        }
        c.addRow(row)
