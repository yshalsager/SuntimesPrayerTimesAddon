...
- يمكنك حفظ عدة مواقع (حتى `10`) وإدارتها من الإعدادات (إضافة/تعديل/حذف/إعادة ترتيب).
- تدعم شاشة Home تبديل الموقع من محدد العنوان/الموقع مع تذكّر آخر اختيار.
- شاشة Days تستخدم نفس الموقع المختار في Home (سياق مشترك)، بما يشمل المنطقة الزمنية وحدود اليوم.
- توجد شاشة بطاقات لكل المواقع المحفوظة تعرض أوقات اليوم كاملة (الصلاة + المكروه + أجزاء الليل).
- يمكن لكل موقع محفوظ أن يستخدم الإعدادات العامة أو ملف حساب مخصصاً له (الطريقة + المكروه + الهجري + إعدادات الفجر/العشاء الإضافيين).
- يوفّر أحداثاً يمكن استخدامها داخل **SuntimesWidget Alarms**:
  - الصلاة (الفجر، الفجر الإضافي ١ اختياري، الضحى، بداية/نهاية نافذة وقت عيد الفطر وعيد الأضحى في أيامهما، الظهر/الجمعة، العصر، المغرب، العشاء، العشاء الإضافي ١ اختياري)
  - حدود/نوافذ أوقات النهي (المكروه)
  - أجزاء الليل (الثلث الأول، منتصف الليل، الثلث الأخير، السدس الأخير)
- يدعم أحداث نافذة صلاة العيد بشكل منفصل لكل عيد:
  - `عيد الفطر: البداية = الشروق + 15 دقيقة، النهاية = الزوال (منتصف النهار الشمسي)`
  - `عيد الأضحى: البداية = الشروق + 15 دقيقة، النهاية = الزوال (منتصف النهار الشمسي)`
...
- يوفّر ثلاثة تقاويم للقراءة فقط يمكن لتطبيقات تقاويم Suntimes اكتشافها:
  ...
  - `makruh`
  - `night`
- يمكن الضغط على الصلاة/حدث الليل داخل Home لفتح محرّر منبّهات المضيف مع اختيار الحدث وسياق الموقع الحالي مسبقاً (منبّهات الأحداث تتتبع تغيّر المواقيت تلقائياً، ويمكن ضبط الإزاحة +/-30 دقيقة من واجهة المضيف).
...
- يوفّر ويدجت على الشاشة الرئيسية ("Prayer Times (Today)") بنفس نموذج بطاقة اليوم.
  - يميز الصلاة القادمة ويعرض عداد الوقت المتبقي مباشرة عندما يدعم المشغل ذلك
- يوفّر إشعاراً دائماً اختيارياً وهادئاً يعرض الصلاة القادمة، والوقت المتبقي عند آخر تحديث، ووقت الصلاة، وموقع Home/Days المختار، وملخص طريقة الحساب.
...
- `notification/*`: إشعار حالة الصلاة الدائم الاختياري
...
- يدعم `eventInfo` تمرير query param إضافي باسم `saved_location_id` لتحديد إظهار/إخفاء الأحداث الاختيارية وعناوينها (الفجر/العشاء الإضافي) حسب ملف الموقع المحفوظ.
- يدعم `eventCalc/<eventId>` تمرير نطاق موقع إضافي عبر query param باسم `saved_location_id`.
- ويدعم أيضاً تمرير حقول الموقع بنفس نمط المضيف داخل `selection` / `selectionArgs` (`latitude` و`longitude` و`altitude` اختياري، بالإضافة إلى `alarm_now`).
- في الأحداث المعتمدة على الشمس، يتم حساب الوقت وفق الموقع الممرر عندما تتوفر هذه الحقول.
- أولوية الحل: `saved_location_id` ثم مطابقة الإحداثيات ثم الموقع العام في المضيف.
...
تحديد نطاق الموقع:
- تدعم استعلامات التقاويم query param إضافي باسم `saved_location_id`.
- وتدعم أيضاً fallback عبر إحداثيات الموقع في `selectionArgs` بنفس نمط المضيف.
- أولوية الحل مطابقة لمزوّد الأحداث: `saved_location_id` ثم الإحداثيات ثم الموقع العام في المضيف.
- بدون مدخلات نطاق الموقع يبقى السلوك الافتراضي مرتبطاً بموقع المضيف (توافق خلفي).
- المحتوى المحدد بنطاق موقع يستخدم المنطقة الزمنية لذلك الموقع داخل `EVENT_TIMEZONE`.

سلوك التقاويم:
- `prayers` يصدّر أحداثاً نقطية للفجر، الفجر الإضافي ١ (عند التفعيل)، الضحى، بداية/نهاية نافذة وقت عيد الفطر وعيد الأضحى، الظهر/الجمعة، العصر، المغرب، العشاء، والعشاء الإضافي ١ (عند التفعيل)
- `makruh` يصدّر أحداثاً على شكل نطاقات لفترات الفجر والشروق والزوال وما بعد العصر والغروب
- `night` يصدّر أحداثاً نقطية للثلث الأول ومنتصف الليل والثلث الأخير والسدس الأخير
...
- `PRAYER_DUHA`
- `PRAYER_EID_FITR_START` (في يوم عيد الفطر فقط)
- `PRAYER_EID_FITR_END` (في يوم عيد الفطر فقط)
- `PRAYER_EID_ADHA_START` (في يوم عيد الأضحى فقط)
- `PRAYER_EID_ADHA_END` (في يوم عيد الأضحى فقط)
...
- المواقع المحفوظة:
  - حفظ حتى `10` مواقع داخل الإضافة.
  - الإضافة إما عبر استيراد موقع المضيف الحالي أو الإدخال اليدوي.
  - تعديل/حذف/إعادة ترتيب المواقع المحفوظة.
  - محدد منطقة زمنية مع بحث وتحذير عند عدم تطابق المنطقة الزمنية.
  - وضع حساب لكل موقع: استخدام الإعدادات العامة أو ملف مخصص للموقع.
- طريقة حساب الصلاة:
  - Presets + Custom
  - زاوية الفجر
...
- الودجت:
  - إظهار/إخفاء صف أوقات النهي
  - إظهار/إخفاء صف أجزاء الليل
  - إظهار تمييز الصلاة القادمة وعداد الوقت المتبقي
  - يستخدم مفاتيح إظهار مستقلة عن التقويم (غير مشتركة)
- الإشعارات:
  - إشعار دائم اختياري لحالة الصلاة القادمة
  - يتبع نفس الموقع المختار في Home/Days
  - يستخدم قناة هادئة منخفضة الأهمية، ولا يتحدث إلا عند الحدود المهمة أو التحديث اليدوي/تحديثات النظام
  - يطلب صلاحية الإشعارات على Android 13+ فقط عند تفعيل الميزة
- المنبّهات والنسخ الاحتياطي:
  - تصدير ملف إعداد منبّهات قابل للاستيراد في المضيف كمنبه أو إشعار أو إشعار سريع (يتضمن الضحى، ويتضمن الإضافات فقط عند تفعيلها)
  - تصدير/استيراد إعدادات الإضافة بصيغة JSON
  - النسخ الاحتياطي يشمل المواقع المحفوظة + الموقع المختار في Home/Days + ملفات الحساب الخاصة بكل موقع + إعداد الإشعار
...
- يمكن ضبط كل ودجت على موقع المضيف (العام) أو على موقع محفوظ واحد، واختيار الودجت مستقل عن اختيار Home/Days.
- يتم تمييز صف الصلاة القادمة، ويستخدم الملخص عداد `Chronometer` مباشراً على المشغلات التي تدعمه.
- الشريط العلوي للتمييز فقط وثابت؛ ليس شريط تقدم.
